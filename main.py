...
def main():
    ...
    with st.form("my_map"):
        ...
        map_display = st.selectbox(
            ...
            ("", "All", "trip", "too close to heavy equipment", "Other"),
        )
        submitted = st.form_submit_button("Submit")

        if submitted:
            if map_display == "All":
                dbcursor.execute("SELECT * FROM {}".format(os.environ.get('TABLE_NAME')))
            else:
                dbcursor.execute("SELECT * FROM {} WHERE Incident_Type='\"{}\"'".format(os.environ.get('TABLE_NAME'), map_display))
            data = [x for x in dbcursor]
            extra = load_data(data)
            # print(extra)

            # if map_display == "All":
            map(extra[["latitude", "longitude"]], csv_location[2], csv_location[1], 10)
            st.write(extra[["latitude", "longitude"]])
